...
- **Partition M property is `MExpression`, not `expression`**: the default `te get <Table>/Partitions/<P>` dump labels the M as `expression`, but `-q expression` errors on **both** `te get` and `te set` with "Property 'expression' not found on MPartition. Did you mean: MExpression?". Use `-q MExpression` for either: `te get <Table>/Partitions/<P> -q MExpression`, `te set <Table>/Partitions/<P> -q MExpression -i - --save` (`-i -` reads the M from stdin, which is the clean way to pass a multi-line expression).
...
- **`te vertipaq` `riViolations` counts relationships, not rows**: the per-table `riViolations` integer is the number of relationships **into** that table (table as the one/dimension side) that currently have unmatched keys; it is NOT a count of orphaned rows or distinct missing keys. A single-digit value can hide millions of orphaned fact rows. To find the actual keys/rows, query with `EXCEPT` (same-type keys) or `RELATED` (mismatched-type keys); a fact-side `CALCULATE(COUNTROWS(Dim))` filter does not propagate across a single-direction many->one relationship and silently returns 0. Full recipe and causes in `references/ri-violations.md`.
